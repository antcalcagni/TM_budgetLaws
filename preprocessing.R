...
# Data --------------------------------------------------------------------
filename = "raw_data/2023_LEGGE 30 dicembre 2023 n213.csv"
...
txts = read.csv(file = filename,header = FALSE,encoding = "UTF-8") #read raw corpus
abbjus = read.csv(file = "utils/abbreviazioni_giuridiche.csv",header = FALSE); #read juridical terms
stpwds = read.delim(file = "utils/stopwords-it_integrate.txt",header = FALSE,sep = "\n",row.names = NULL,stringsAsFactors = FALSE,fileEncoding = "ISO-8859-1");
polirems = read.delim(file = "utils/parole_tecniche-it.txt",header = FALSE,sep = "\n",row.names = NULL,stringsAsFactors = FALSE);

str(txts)
for(i in 1:length(txts[,1])){
  txts[i,2] = paste(txts[i,1],txts[i,2],sep = ". ")
}
txts = data.frame(V1=txts[,-1]);
str(txts)
...
vbs = c("essere","avere","dovere","potere","fare","apportare","provvedere","definire","denominare")
