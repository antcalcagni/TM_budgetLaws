...
FLSA_core = function(X=NULL,N=NULL,M=NULL,K=NULL,type = c("svd","nnmf","binary"),verbose=FALSE){
  ...
  U = switch(mfact,
             svd = irlba::irlba(A = X, nv = 2)$u,
             nnmf = nnTensor::NMF(X = X,J = 2)$U,
             binary = nmfbin::nmfbin(X = X,k = 2)$W)
